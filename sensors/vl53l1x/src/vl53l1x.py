...
# https://github.com/adafruit/Adafruit_CircuitPython_VL53L1X/tree/ca9a18db70b14353301af435cba397844dbfba73
...
def readReg(reg):
    return i2c.readfrom_mem(DEVICE_ADDR, reg, 1, addrsize=16)[0]

def writeReg(reg, value):
    return i2c.writeto_mem(DEVICE_ADDR, reg, bytes([value]), addrsize=16)
	
def readReg16Bit(reg):
    data = i2c.readfrom_mem(DEVICE_ADDR, reg, 2, addrsize=16)
    return (data[0]<<8) + data[1]

def writeReg16Bit(reg, value):
    i2c.writeto_mem(DEVICE_ADDR, reg, bytes([(value >> 8) & 0xFF, value & 0xFF]), addrsize=16)
...
def reset():
     ...
     utime.sleep_ms(100)  # machine.lightsleep(100)
     writeReg(0x0000, 0x01)

def __init__():
     reset()
     utime.sleep_ms(1)     #machine.lightsleep(1)
     if read_model_id() != 0xEACC:
           raise RuntimeError('Failed to find expected ID register values. Check wiring!')
     i2c.writeto_mem(DEVICE_ADDR, 0x2D, VL51L1X_DEFAULT_CONFIGURATION, addrsize=16)
     writeReg16Bit(0x001E, readReg16Bit(0x0022) * 4)
     utime.sleep_ms(200)

def read():
    data = i2c.readfrom_mem(DEVICE_ADDR, 0x0089, 17, addrsize=16)
    range_status = data[0]
    stream_count = data[2]
