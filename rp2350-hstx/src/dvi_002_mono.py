#
# file: dvi_002_mono.py
#
#  V0.01  (2026/8/9)
#   monochrome version
#   display size: DVI (640x480)
#   display pattern: vertial stripes with black/white
#   using 2 DMA, 
#      DMA0 : transfer VSYNC
#      DMA1 : transfer HSYNC + DISPLAY DATA
...
def init_vga_HSYNC_display():

    data_size = 8 * VGA_HEIGHT
...
#
vga_buffer_VSYNC = init_vga_VSYNC()
vga_buffer_HSYNC_disp = init_vga_HSYNC_display()
...
def dma1_itr_handler(arg):
    dma1.read = vga_buffer_HSYNC_disp

def setup_dma(dma0, dma1, buffer0, buffer1):
    ...
    dma0.config(read=buffer0, write=HSTX_FIFO_ADDR, ctrl=dma0_ctrl, trigger=False, count=len(buffer0))

    dma1_ctrl = dma1.pack_ctrl(size=2, inc_read=True, inc_write=False, treq_sel=52, irq_quiet=False, chain_to=dma0.channel)
    dma1.config(read=buffer1, write=HSTX_FIFO_ADDR, ctrl=dma1_ctrl, trigger=False, count=len(buffer1))
                                                                                           

setup_dma(dma0, dma1, vga_buffer_VSYNC, vga_buffer_HSYNC_disp)
