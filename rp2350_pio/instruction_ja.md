...
- PIOプログラミングを行う上で、StateMachine コンストラクタ、PIO クラス、`asm_pio` の仕様理解が必要です。i
...
- `clear`: IRQフラグをクリアする  
- `block`: クリアされるまで待つ  (irq命令でIRQを発行した後、受理された相手方によってIRQがクリアされるのを待つ)
...
irq(rel(3))
```
- SM0でirq(rel(3))とすると、絶対化（3+0)でIRQ：3が発行される。SM1でirq(rel(3))とすると絶対化(3+1)で、IRQ:4 が発生される。
- rel()の表記があるのは、同じソースコードを異なるStatemachine間で共有する際、IRQ番号が自動的にオフセットされるのでIRQの重複を避けることが可能になる。
- MicroPythonにIRQを送りたい場合、指定できるIRQ番号は0から3まで。4以上を指定してもCPU側からは見えないためMicroPythonでは受け取れない(RP2040の制限)(RP2350ではこの制限は解消された)。
- MicroPythonにIRQを送りたい場合、irq(rel(0))と書くと、Statemachine番号でIRQ番号が自動的にオフセットされるので、この書き方が無難。
...
#### 命令セット　（注：RP2350版、RP2040では使えない命令が含まれます)
